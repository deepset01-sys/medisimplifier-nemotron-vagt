...
**Out-of-sample check.** A stratified 50/50 split under the *deployed* gate prompt ([`results/split_half_validation.json`](results/split_half_validation.json)) confirms the diagnosis ΔΦ_V holds on held-out data (**+0.052 [+0.030, +0.075]**, CI excludes 0) — and only diagnosis survives: dose, negation, and lateral all straddle zero, so those calibration-prompt gains were artifacts and the VAGT lift is concentrated on the one stratum where the incumbents are genuinely blind.
