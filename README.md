...
*Each candidate added to the Llama+Qwen incumbent, on the **v2 clean 240-item diagnosis stratum** (120 hand-verified τ=1 drops + 120 paired τ=0 controls; deployed prompt; paired 1000-iteration bootstrap 95% CI, SEED=42; [`results/judgebench_v2_pool_table.json`](results/judgebench_v2_pool_table.json), [`scripts/run_pool_judgebench_v2.py`](scripts/run_pool_judgebench_v2.py)). **DeepSeek and gpt-oss are statistically tied at the top** (overlapping CIs) and roughly double the Nemotrons; **gpt-oss is recommended** on the reliability tie-break (0 errors, 73% vs 66% specificity — see below). gemma straddles zero (null). These are per-candidate CIs vs the incumbent, not paired between-candidate tests. **Scope:** only the diagnosis stratum was rebuilt on clean labels — the v1 dose/negation/lateral columns are not yet re-run, so they are omitted here. (v1 reported this table on a 708-item automated benchmark whose diagnosis labels were ~85% mislabeled; those numbers are superseded — see "The finding" above.) Full per-candidate verdicts in [`results/judgebench_v2_pool_<slug>.json`](results/).*
...
- **On clean labels the recommendation changes — gpt-oss-120b, not Nemotron Nano.** v1 recommended Nano, but that rested on the contaminated 708-item pool, where the top candidates sat in a ~+0.07 three-way tie. Rebuilt on the 120-item hand-verified v2 diagnosis stratum, two candidates pull clearly ahead: **gpt-oss-120b +0.1220 [+0.1000, +0.1416]** and **DeepSeek-V4-Flash +0.1238 [+0.1024, +0.1440]** — roughly *double* Nano's **+0.0765 [+0.0516, +0.0992]** (and Ultra's +0.0781), with confidence intervals that clear both. gpt-oss and DeepSeek are **statistically tied** on ΔΦ_V (overlapping CIs; ~+0.104 vs +0.101 after subtracting the permutation-null baseline), so the pick is broken on **reliability, not margin**: gpt-oss returned **0 errors** (DeepSeek errored on 7 of 240 items) and the **higher specificity — 73% vs 66%** (it false-flags fewer clean summaries). The lift is detection-specific (permutation-null p = 0.001) and holds out-of-sample (split-half **+0.122 / +0.122**, POSITIVE in both halves). Two honest caveats: gpt-oss-120b is **4× Nano's size (120B vs 30B)** — clean labels trade cost for materially higher detection, reversing v1's "smallest-and-cheapest" tie-break — and this recommendation is **diagnosis-stratum only**; the v1 four-stratum collateral comparison (dose/negation/lateral) has not been re-run on clean data, so that cross-stratum tie-break no longer applies here.
...
The **[`/v1/audit_panel`](#b3-api-contract)** endpoint runs exactly this analysis on any incumbent panel + candidate pool, returning the recommended judge, its ΔΦ_V, and a bootstrap CI. The **v2 recommendation is gpt-oss-120b (+0.1220, CI [+0.1000, +0.1416])** — an offline recompute on the clean stratum in [`results/audit_panel_receipt_v2_diagnosis.json`](results/audit_panel_receipt_v2_diagnosis.json). The live endpoint still serves the **v1** pool, so its captured receipt ([`results/audit_panel_live_receipt.json`](results/audit_panel_live_receipt.json)) records the now-**superseded** v1 pick (Nemotron Nano, +0.0706) and is preserved unchanged as a genuine capture. The endpoint re-ranks **pre-computed** verdict files (`audit_pool/verdicts/`) on CPU in milliseconds; it does not call the candidate models live, so adding a genuinely new judge means generating its verdicts first (`gen_pool_verdicts.py`).
...
> **Out-of-sample under the deployed gate prompt** (stratified split-half, test half; [`results/split_half_validation.json`](results/split_half_validation.json)): the diagnosis blind spot **persists but narrows** — Nemotron **56%**, Llama **17%**, Qwen **23%** (vs the calibration-prompt 68% / 14% / 7% above). The incumbents still miss ~77–83% of diagnosis drops; the gap is smaller under the deployed prompt but the finding holds. On dose/lateral/negation all three judges are comparable, so the blind spot is **diagnosis-specific**.
...
**Why not just consensus accuracy?** Because it can point the wrong way. The obvious baseline, when τ is known, is whether the panel's majority vote matches truth. Under that metric, adding Nemotron *lowers* balanced accuracy on the diagnosis stratum — **60.5% → 58.3%** ([`results/consensus_accuracy.json`](results/consensus_accuracy.json), deployed gate prompt) — because a majority vote lets the two blind incumbents outvote the one judge that catches the drop (the same reason the deployed rule is **not** a majority vote). The variance decomposition is threshold-free and points correctly: on diagnosis it shows Nemotron **reduces the shared bias σ²_B** (0.289 → 0.232, gate prompt; cf. the calibration-prompt 0.347 → 0.229 in the opening) and raises Φ_V (**+0.044**); on dose and lateral, where the incumbents already see, it *raises* σ²_B and *lowers* Φ_V — correctly flagging the collateral over-flagging that majority-vote accuracy instead rewards (+3.5% / +4.2%). Consensus accuracy tells you whether one fixed voting rule got lucky on the panel you have; Φ_V tells you whether the consensus is *systematically* closer to truth — the quantity judge-selection needs. (Honest caveat: the balanced-accuracy comparison also shifts the vote threshold as the panel grows — "either of two" vs "two of three" — so σ²_B, being threshold-free, is the clean signal.)
