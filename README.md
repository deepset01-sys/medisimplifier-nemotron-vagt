...
Two audiences, two tracks. **Track A — Research Design** gives the estimand, benchmark, protocol, VAGT derivation, per-judge calibration, the inversion, and threats to validity. **Track B — Product Design** gives the `POST /v1/simplify` contract, the decision rule with Nemotron as the diagnosis-drop tripwire, measured operating characteristics (~1-in-3 DISAGREEs is a false alarm (34.7% measured, see B5); ~27 s per request), Nebius deployment, and known issues — including that the gate's Qwen judge was briefly swapped mid-project (Qwen3-32B → Qwen3-30B-A3B) but is now restored to Qwen3-32B and recalibrated (see B8). A 1,001-output student self-audit and dual-auditor diagnosis-retention review (Claude Sonnet 5 + Gemini 2.5 Pro) confirmed that the gate's 52% flag rate reflects simplification trade-offs, not systematic diagnosis loss: independent auditors agreed on genuine drops in only 2 of 20 flagged cases (see B5). This is a research prototype: unauthenticated, not clinician-validated, and not for real patient data.
