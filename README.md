...
*Each candidate added to the Llama+Qwen incumbent; diagnosis is the panel's blind stratum, dose the one where the incumbents already see well. These are point estimates on 708 items; the top three sit inside the Nano reference's 95% CI [+0.055, +0.087], but we did not compute paired per-candidate CIs, so we do not claim they are statistically indistinguishable — only that their point estimates are close. Full per-candidate verdicts in [`audit_pool/verdicts/`](audit_pool/verdicts/), receipts in [`audit_pool/candidates.yaml`](audit_pool/candidates.yaml).*
