...
Patients go home with discharge summaries written for clinicians — dense with abbreviations and diagnoses most people cannot read. **MediSimplifier v2** rewrites them into plain language and returns a safety verdict: two standard judges (Llama-3.3-70B, Qwen3-32B) miss 77–93% of silently dropped diagnoses; NVIDIA Nemotron Nano catches 56–68% — and the agreement metric that would have rejected this fix went negative when we added it.
...
*Each candidate added to the Llama+Qwen incumbent; diagnosis is the panel's blind stratum, dose the one where the incumbents already see well. Point estimates on 708 items, each with a paired 1000-iteration bootstrap 95% CI ([`results/pool_candidate_cis.json`](results/pool_candidate_cis.json), [`scripts/compute_pool_cis.py`](scripts/compute_pool_cis.py)): the top three (Nano/Ultra/gpt-oss) overlap heavily and are statistically indistinguishable on diagnosis, while gemma straddles zero. These are per-candidate CIs (vs the incumbent), not paired between-candidate tests. Full per-candidate verdicts in [`audit_pool/verdicts/`](audit_pool/verdicts/), receipts in [`audit_pool/candidates.yaml`](audit_pool/candidates.yaml).*
...
**Robustness — the null-rater control.** Is the diagnosis lift real detection, or just a flag-rate artifact? We added two synthetic null raters to the Llama+Qwen panel and scored them identically ([`scripts/compute_null_baseline.py`](scripts/compute_null_baseline.py), [`results/null_baseline_cis.json`](results/null_baseline_cis.json)): a **constant-UNSAFE** rater (flags everything) and a **random 47%-flagger** (Nemotron's approximate UNSAFE rate, tracking nothing). Honestly, **both raise diagnosis ΔΦ_V** — constant +0.0486 [+0.026, +0.067], random +0.0381 [+0.021, +0.053], CIs excluding zero — because the stratum is corrupted-heavy and the incumbents are so blind there that *any* upward push on consensus helps. So the diagnosis number **alone** is not detection-specific. But the nulls pay for it: they **crater** dose (−0.11 to −0.17), lateral, and negation by flagging clean items, ending **net-negative** across the four strata (mean ΔΦ_V −0.063 and −0.037). **Nemotron is the only rater that lifts the blind stratum *without* collateral damage — the only one net-positive (mean +0.037).** That selectivity is exactly what a brute-force flagger cannot fake, and exactly why the selector ranks by blindest-stratum lift **plus a least-collateral tie-break**: strip the collateral term and a constant-UNSAFE rater ranks near the top. The null-rater baseline is the control showing the method rewards *detection*, not flag rate.

**Out-of-sample check** (split-half, deployed prompt): diagnosis ΔΦ_V **+0.052 [+0.030, +0.075]** holds on held-out data; only diagnosis survives (see B5 for full split-half results).

The **[`/v1/audit_panel`](#b3-api-contract)** endpoint runs exactly this analysis on any incumbent panel + candidate pool, returning the recommended judge, its ΔΦ_V, and a bootstrap CI — live receipt (recommends Nemotron Nano, +0.0706, CI [+0.055, +0.087]) in [`results/audit_panel_live_receipt.json`](results/audit_panel_live_receipt.json). The endpoint re-ranks **pre-computed** verdict files (`audit_pool/verdicts/`) on CPU in milliseconds; it does not call the candidate models live, so adding a genuinely new judge means generating its verdicts first (`gen_pool_verdicts.py`).
...
9. **Reference-authorship self-preference.** The 708-item calibration judged Nemotron Super references (A2). Nemotron Nano shares model family with the reference author; self-preference would make Nano more lenient on same-family text, biasing diagnosis recall downward — understating, not inflating, the catch-rate advantage. The deployed gate panel (Llama + Qwen + Nano) has only one Nemotron-family judge; the pool experiment adds two more (Super, Ultra) as candidates, where the confound is sharpest.
