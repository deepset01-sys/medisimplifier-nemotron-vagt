...
- ⬜ Demo caption (app/demo.jsx): "the tool picks gpt-oss" — needs construction-circularity caveat (A8 threat 10)
      (2026-09-27: post-hoc tie-break disclosure + budget note added to the caption; circularity caveat still missing)
- ⬜ Optional: full 240-row DeepSeek re-run at a matched budget (`scripts/check_deepseek_budget_v2.py --phases B`) —
      only the 7 ERROR items were re-run; the other 233 DeepSeek verdicts are from the 4000-token pool run.
- ⬜ README lede contradicts A6 (line 207): line 13 says the agreement metric "went negative"; lines 19 and 77
      overstate κ's direction ("the opposite of what Cohen's κ predicts" / "demonstrating that Cohen's κ … moves in
      the wrong direction"). Must read: "Δκ fell slightly (−0.036, 95% CI [−0.131, +0.057], not significant)"
      (results/judgebench_v2_agreement_recompute.json; same precision as A6).
- ⬜ README A4 and A5: automated-pass figures are unlabelled (A4 σ²_τ 0.243 at p=0.41 and complete-case
      n=280/295/341/333; A5 top table 84.2% / 31.7% / 55.9%) — add an "automated-pass figures" label to both.
- ⬜ README "What's new in v2" table: missing a JudgeBench v2 row (412 items, chambul/MedSimp-JudgeBench-v2).
- ⬜ scripts/run_panel_judgebench_v2.py depends on untracked local files (results/student_predictions.json and the
      v1-repo calibration_verdicts.json via a local D:\ path) — the v2 panel cannot be re-run from a fresh clone /
      the public dataset; switch its source text to the HF v2 `input` field.
...
Finding 1: ✅ DONE — fully rewritten on v2 figures (63e1a28)
  ...
  - 2026-09-27: recommendation sentence now says gpt-oss is tied with DeepSeek and the tie-break was added after the result
...
How the v2 pick is made (checked 2026-09-27): gpt-oss +0.1220 and DeepSeek +0.1238 round into the same 0.01
TIE_BAND and collateral is 0 for both (one stratum), so the fewest-ERROR key decides (0 vs 7); specificity is never
reached. DeepSeek's 7 ERRORs are max_tokens truncations (DeepSeek ran at 4000, gpt-oss at 8000). With those 7 items
re-run at 8000, DeepSeek has 0 errors and ΔΦ_V +0.1208 [+0.1001, +0.1410] on all 240, and gpt-oss is still
selected — on specificity (88/120 vs 79/120). The error-count tie-break was added after the v2 result was known;
that is now disclosed in README/demo/app/NEMOTRON_INSIGHTS/receipt.
...
### Completed this session (2026-09-27) — DeepSeek budget check + tie-break/specificity corrections
- ✅ DeepSeek's 7 ERROR verdicts are a TOKEN-BUDGET ARTIFACT (scripts/check_deepseek_budget_v2.py →
   results/judgebench_v2_deepseek_budget_check.json). Live re-run 2026-09-26 22:04–22:06 UTC: the 7 committed ERROR
   items × max_tokens {4000, 8000} × 3 repeats = 42 calls, deployed JUDGE_PROMPT + safety_gate payload, 300 s client
   timeout. Failures: 5/21 @4000, 1/21 @8000 — ALL finish_reason=length, empty content, reasoning_tokens = the whole
   budget (the gate parses "" as ERROR, no retry). No timeouts, no HTTP errors (slowest call 46 s < the gate's 60 s).
   Per item: 614/τ0 fails 3/3 @4000 and 1/3 @8000; hf195 2/3 @4000; the other five answered 3/3 at both budgets
   (their original errors were intermittent — the pool file stores only the verdict, so not provable per item).
   Budget reduces but does not remove truncation (cf. v1: residual truncations at 16k).
- ✅ Selector effect: filling the 7 items with their @8000 majority → DeepSeek 0 errors, recall 119/120, spec 79/120,
   ΔΦ_V +0.1208 [+0.1001, +0.1410] → still gpt-oss (tie-break falls through to specificity). @4000 fill → 1 error,
   +0.1216 → gpt-oss. Offline bound over all 2^7 = 128 SAFE/UNSAFE fills: gpt-oss 127/128; the one exception
   (all 7 answered correctly) puts DeepSeek at 0.1254, which rounds into the 0.13 band (TIE_BAND rounding — audit #20).
   Minimal counterfactual only: the other 233 DeepSeek verdicts are from the 4000 run (full re-run = --phases B, not run).
- ✅ Thinking-flag live evidence (audit #1 / open #19): DeepSeek-V4-Flash reasoned on 36/36 answered calls
   (reasoning_tokens > 0) although the payload carries "extra_body": {"enable_thinking": False} posted via requests.
   The flag AS SENT does not stop DeepSeek's reasoning; "not delivered" vs "ignored" is still not distinguishable.
   Nemotron @1024-empty / Nano claims still uncaptured.
- ✅ Docs now match the selector (error count decides; specificity not reached in the served data; the tie-break was
   added after the result): README L38 + L44 (also "CIs that clear both" fixed — DeepSeek's CI clears both Nemotrons,
   gpt-oss's touches Ultra's, 0.1000 vs 0.1003; budget-artifact sentence), L74 ("leads" → tie), L349, L445, API table
   L578, A9 commands L327-328, scripts list L837; app/demo.jsx caption; app/README.md; docs/NEMOTRON_INSIGHTS.md;
   results/audit_panel_receipt_v2_diagnosis.json (reason + note corrected; original wording kept in
   correction_2026_09_27 — "best specificity" was false: gemma 91/120 = 76%); audit_pool_v2/candidates.yaml header +
   scripts/build_audit_pool_v2.py (rebuild byte-identical, verify OK). 25/25 tests; demo builds.
- ⚠️ The Nebius key was pasted into chat on 2026-09-27 and stored in the gitignored .env (untracked) → rotate.
...
- ✅ Selector/schema/router/cpu_endpoint/Dockerfile.cpu + tests (6948067):
   ...
   · NEW tie-break inside a TIE_BAND tie, AFTER least-collateral: fewest ERROR verdicts, then
     ...
     already stated in audit_panel_receipt_v2_diagnosis.json; disclose as such [✅ disclosed 2026-09-27: README
     L38/L44/L349/L445 + API table L578, demo caption, app/README, NEMOTRON_INSIGHTS, receipt]. Without it
...
#10/#12 billing export · #19 raw API captures (Nemotron @1024 empty; enable_thinking ineffective — partial live evidence 2026-09-27: DeepSeek
reasons with the flag sent, 36/36 answered calls, results/judgebench_v2_deepseek_budget_check.json; Nemotron still uncaptured) · #20
...
OWNER STILL OPEN: rotate the Nebius key (pasted into chat again 2026-09-27; now in the gitignored .env — replace or
delete after rotating) AND the HuggingFace token; stop the dedicated Qwen3-32B + Llama-3.3-70B
